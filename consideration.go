...
type Consideration struct {//TODO: Beware of reordering struct fields, IDs seem to be sensitive to field order.
	...
	Matures   int64             `json:"matures,omitempty"` // pass height. if set consideration can't be tracked before
	Expires   int64             `json:"expires,omitempty"` // pass height. if set consideration can't be tracked after
...
// used to recognise the tracker for tracking the pass.
...
// IsMature returns true if the consideration can be tracked at the given height.
...
// IsExpired returns true if the consideration cannot be tracked at the given height.
