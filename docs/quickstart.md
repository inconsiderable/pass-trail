...
Trackers running in the client are responsible for tracking new passes in coordination with the network. When a tracker running on your local node tracks a new pass it will automatically create a consideration on the network sending the pass point to one of your keyholder-managed public keys. 
## Pre-requisites
...
These keys will later be used to send and receive considerations on the network from tracker instances or other keyholders.
