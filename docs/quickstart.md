...
Using pass-trail requires two components: a client and a keyholder.
...
The client is the component responsible for maintaining a peering connection to the network (i.e. running a node) and trailing. The client uses a peer discovery protocol to bootstrap itself onto the network and then cooperates with other nodes to manage the distributed ledger.

Trailers running in the client are responsible for trailing new passes in coordination with the network. When a trailer running on your local node trails a new pass it will automatically create an consideration on the network sending the pass point to one of your keyholder-managed public keys. 
## Pre-requisites
...
$ go get -v github.com/inconsiderable/pass-trail/client github.com/inconsiderable/pass-trail/keyholder
$ go install -v github.com/inconsiderable/pass-trail/client github.com/inconsiderable/pass-trail/keyholder
```
...
These keys will later be used to send and receive considerations on the network from trailer instances or other keyholders.
...
Given the newly created keyfile, we're ready to connect to run the client and begin trailing:

```
$ client -datadir pass-node -keyfile keys.txt -numtrailers 4 -upnp
```

!> Note: To enable constant trailing, make sure the `client` process stays running in either `screen` or another durable session.
