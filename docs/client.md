...
Usage of /home/passtrail/go/bin/client:
  ...
  -keyfile string
        Path to a file containing public keys to use when trailing
  -memo string
        A memo to include in newly trailed passes
  -noaccept
        Disable inbound peer connections
  -noirc
        Disable use of IRC for peer discovery
  -numtrailers int
        Number of trailers to run (default 1)
  -peer string
  ...
  -pubkey string
        A public key which receives newly trailed pass points
  -tlscert string
...
The client requires a data dir for storage of the pass-trail and general metadata as well as one or more public keys to send pass points to upon trailing. Otherwise, running the client is as simple as:

```
$ client -datadir pass-data -keyfile keys.txt -numtrailers 2
```
...
In order to effectively trail, you'll typically want to run one trailer per CPU core on your machine. This is configured via the `-numtrailers` param, like so:

```
$ client ... -numtrailers 4
```

To run a trailer-less node, you can pass `0` as the number of trailers like so:

```
$ client ... -numtrailers 0
```

### Configuring Keys

The client supports two modes of pass point considerations for trailing: single key and key list targets.
