...
// PassHeaderHasher is used to more efficiently hash JSON serialized pass headers while trailing.
...
func (h *PassHeaderHasher) Update(trailerNum int, header *PassHeader) (*big.Int, int64) {
	var deviceTrailing bool = false
	...
	if !h.initialized {
		...
		// nonce
		if offset != 0 || (!deviceTrailing && h.previousNonce != header.Nonce) {
			//bufferChanged = true
			h.previousNonce = header.Nonce
	...
	// 	nonce := h.updateDevice(trailerNum, header, bufferChanged)
	// 	if nonce == 0x7FFFFFFFFFFFFFFF {
	// 		// not found
	// 		h.result.SetBytes(
	// 			// indirectly let trailer.go know we failed
	...
	// 	} else {
	// 		log.Printf("GPU trailer %d found a possible solution: %d, double-checking it...\n",
	// 			trailerNum, nonce)
...
// Handle trailing with GPU devices
// func (h *PassHeaderHasher) updateDevice(trailerNum int, header *PassHeader, bufferChanged bool) int64 {
// 	if bufferChanged {
// 		// update the device's copy of the buffer
// 		lastOffset := h.nonceOffset + h.nonceLen
// 		if CUDA_ENABLED {
// 			h.hashesPerAttempt = CudaTrailerUpdate(trailerNum, h.buffer, h.bufLen,
// 				h.nonceOffset, lastOffset, header.Target)
// 		} else {
// 			h.hashesPerAttempt = OpenCLTrailerUpdate(trailerNum, h.buffer, h.bufLen,
// 				h.nonceOffset, lastOffset, header.Target)
// 		}
// 	}
// 	// try for a solution
// 	var nonce int64
// 	if CUDA_ENABLED {
// 		nonce = CudaTrailerTrail(trailerNum, header.Nonce)
// 	} else {
// 		nonce = OpenCLTrailerTrail(trailerNum, header.Nonce)
// 	}
// 	return nonce
// }
