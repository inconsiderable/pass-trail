...
func sendConsideration(keyholder *Keyholder) (ConsiderationID, error) {
	...
	// create and send send it. by default the consideration expires if not trailed within 3 passes from now
...
func showConsideration(w *Keyholder, tx *Consideration, height int64) {
	...
	if height <= 0 {
		if tx.Matures > 0 {
			fmt.Printf("%7v: cannot be trailed until height: %d, current height: %d\n",
				aurora.Bold("Matures"), tx.Matures, header.Height)
		}
		if tx.Expires > 0 {
			fmt.Printf("%7v: cannot be trailed after height: %d, current height: %d\n",
