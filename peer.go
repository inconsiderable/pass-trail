...
// Peers could be fully validating and trailing nodes or simply keyholders.
type Peer struct {
	conn                          *websocket.Conn
	genesisID                     PassID
...
func (p *Peer) run() {
	...
	go func() {
		...
		// check to see if we need to update work for trailers
...
func (p *Peer) onGetProfile(pubKey ed25519.PublicKey, outChan chan<- Message) error {
	...
	_, catchment := catchmentFromPubKey(pk, p.indexer.catchments.Values())
...
func (p *Peer) onGetGraph(pubKey ed25519.PublicKey, outChan chan<- Message) error {
	...
	passGraph := p.indexer.txGraph.ToDOT(pk, p.indexer.catchments.Values())
...
// Create a new work pass for a trailing peer. Called from the writer goroutine loop.
...
// Handle a submission of trailing work. Called from the writer goroutine loop.
