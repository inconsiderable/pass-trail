...
// Peers could be fully validating and tracking nodes or simply keyholders.
type Peer struct {
	conn                          *websocket.Conn
	genesisID                     PassID
...
func (p *Peer) onGetProfile(pubKey ed25519.PublicKey, outChan chan<- Message) error {
	...
	_, focale, _ := focaleFromPubKey(pk, p.indexer.focales.Values())
...
func (p *Peer) onGetGraph(pubKey ed25519.PublicKey, outChan chan<- Message) error {
	...
	passGraph := p.indexer.txGraph.ToDOT(pk, p.indexer.focales.Values())
