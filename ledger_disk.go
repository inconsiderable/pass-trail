...
func (l LedgerDisk) GetTrailTip() (*PassID, int64, error) {
	return getTrailTip(l.db)
}

// Sometimes we call this with *leveldb.DB or *leveldb.Snapshot
func getTrailTip(db leveldb.Reader) (*PassID, int64, error) {
	// compute db key
	key, err := computeTrailTipKey()
	if err != nil {
		return nil, 0, err
	}
	...
	// decode the tip
	id, height, err := decodeTrailTip(ctBytes)
	if err != nil {
		return nil, 0, err
	}
...
// ConnectPass connects a pass to the tip of the pass trail and applies the considerations to the ledger.
func (l LedgerDisk) ConnectPass(id PassID, pass *Pass) ([]ConsiderationID, error) {
	// sanity check
	tipID, _, err := l.GetTrailTip()
	if err != nil {
		return nil, err
	}
	...
	// set this pass on the main trail
	key, err = computeBranchTypeKey(id)
	if err != nil {
		return nil, err
	}
	...
	ctBytes, err := encodeTrailTip(id, pass.Header.Height)
	if err != nil {
		return nil, err
	}
...
// DisconnectPass disconnects a pass from the tip of the pass trail and undoes the effects of the considerations on the ledger.
func (l LedgerDisk) DisconnectPass(id PassID, pass *Pass) ([]ConsiderationID, error) {
	...
	// set the previous pass as the trail tip
	key, err = computeTrailTipKey()
	if err != nil {
		return nil, err
	}
	ctBytes, err := encodeTrailTip(pass.Header.Previous, pass.Header.Height-1)
	if err != nil {
		return nil, err
	}
	batch.Put(key, ctBytes)
...
func (l LedgerDisk) GetPublicKeyImbalances(pubKeys []ed25519.PublicKey) (
	...
	// get the trail tip
	tipID, tipHeight, err := getTrailTip(snapshot)
	if err != nil {
		return nil, nil, 0, err
	}
...
func (l LedgerDisk) GetPublicKeyImbalanceAt(pubKey ed25519.PublicKey, height int64) (int64, error) {
	_, currentHeight, err := l.GetTrailTip()
	if err != nil {
		return 0, err
	}
...
func computeTrailTipKey() ([]byte, error) {
	key := new(bytes.Buffer)
	if err := key.WriteByte(trailTipPrefix); err != nil {
		return nil, err
	}
	return key.Bytes(), nil
}
...
func encodeTrailTip(id PassID, height int64) ([]byte, error) {
...
func decodeTrailTip(ctBytes []byte) (*PassID, int64, error) {
