...
// GetWorkMessage is used by a trailing peer to request trailing work.
// Type: "get_work"
...
// WorkMessage is used by a client to send work to perform to a trailing peer.
// The timestamp and nonce in the header can be manipulated by the trailing peer.
// It is the trailing peer's responsibility to ensure the timestamp is not set below
...
// SubmitWorkMessage is used by a trailing peer to submit a potential solution to the client.
// Type: "submit_work"
...
// SubmitWorkResultMessage is used to inform a trailing peer of the result of its work.
