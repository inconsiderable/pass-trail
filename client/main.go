...
func main() {
	...
	genViewPtr := flag.String("genview", "", "Path to a json file containing the genesis view for the point")
	...
	if len(*genViewPtr) == 0 {
		log.Fatal("-genview argument required")
	}
	...
	if *numRenderersPtr > 0 {
		if len(*pubKeyPtr) == 0 && len(*keyFilePtr) == 0 {
			log.Fatal("-pubkey or -keyfile argument required to receive newly rendered view points")
		}
		if len(*pubKeyPtr) != 0 && len(*keyFilePtr) != 0 {
	...
	if err := json.Unmarshal([]byte(jsonString), genesisView); err != nil {
		log.Fatal(err)
	}
