...
func main() {
	...
	// flags
	pubKeyPtr := flag.String("pubkey", "", "A public key which receives newly trailed pass points")
	dataDirPtr := flag.String("datadir", "", "Path to a directory to save pass trail data")
	memoPtr := flag.String("memo", "", "A memo to include in newly trailed passes")
	...
	numTrailersPtr := flag.Int("numtrailers", 1, "Number of trailers to run")
	...
	keyFilePtr := flag.String("keyfile", "", "Path to a file containing public keys to use when trailing")
	...
	// load public keys to trail to
	var pubKeys []ed25519.PublicKey
	if *numTrailersPtr > 0 {
		if len(*pubKeyPtr) == 0 && len(*keyFilePtr) == 0 {
			log.Fatal("-pubkey or -keyfile argument required to receive newly trailed pass points")
		}
		if len(*pubKeyPtr) != 0 && len(*keyFilePtr) != 0 {
	...
	if *numTrailersPtr > 0 {
		hashUpdateChan := make(chan int64, *numTrailersPtr)
		// create and run trailers
		for i := 0; i < *numTrailersPtr; i++ {
			trailer := NewTrailer(pubKeys, *memoPtr, passStore, txQueue, ledger, processor, hashUpdateChan, i)
		...
		log.Println("Trailing is currently disabled")
	}
