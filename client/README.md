...
A client is a fully validating and trailing peer-to-peer node in the passtrail network.
...
- **pubkey** - This is a public key which receives your node's trailing points. You can create one with the [keyholder software](https://github.com/inconsiderable/pass-trail/tree/master/keyholder).
...
- Attempt to trail new passes and share them with connected peers.
- Validate and share new passes and considerations with peers.

## Other options
- **memo** - A memo to include in newly trailed passes.
...
- **numtrailers** - Number of trailer threads to run. Default is 1.
- **noirc** - Disable use of IRC for peer discovery. Default is true.
- **noaccept** - Disable inbound peer connections.
- **keyfile** - Path to a file containing public keys to use when trailing. Keys will be used randomly.
