...
func inflateNodes(pubKey string) (bool, string, []string, string) {
	...
	return true, focale, nodes, notes
}
		...
		if strings.TrimRight(notes, "+") == "" && len(nodes) == 2 {
			...
			for i := 0; i < len(nodes); i++ {
				...
				for j := i + 1; j < len(nodes); j++ {
					...
					idx.txGraph.Link(padTo44Characters(trimmedNode), padTo44Characters(trimmedNext), incrementBy)
				}
		
				for k := 0; k < len(nts); k++ {
					note := nts[k]
					idx.txGraph.Link(padTo44Characters(trimmedNode), padTo44Characters(note), incrementBy)
				}		
			}
