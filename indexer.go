...
func (idx *Indexer) indexConsiderations(pass *Pass, id PassID, increment bool) {
	...
	if sIndex, ok := idx.txGraph.index[padTo44Characters("0")]; ok {		
		prevWeight = idx.txGraph.nodes[sIndex].inbound
	}
	...
	if increment {
		incrementBy = 1
		if pass.Header.Height > 0 {			
			idx.txGraph.Reassign(padTo44Characters("0"), padTo44Characters(prevHeight+"/")) //archive and reset
			idx.txGraph.Link(padTo44Characters(prevHeight+"/"), padTo44Characters("0"), prevWeight/2)
		}
	} else {
		//Pass disconnected: Reverse all applicable considerations from the graph
		incrementBy = -1
		if pass.Header.Height > 0 {
			idx.txGraph.Reassign(padTo44Characters(prevHeight+"/"), padTo44Characters("0"))//undo archive
			idx.txGraph.Link(padTo44Characters(prevHeight+"/"), padTo44Characters("0"), 0)
		}
	}
	...
	for i := 0; i < len(pass.Considerations); i++ {
		...
		*/
		...
		idx.txGraph.Link(conFor, padTo44Characters("0"), weight/2)
	}
}
