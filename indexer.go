		...
		*/
		if ok, focale, catchments := focaleFromPubKey(conFor, idx.focalPoints.Values()); ok && nodesOk {
			...
			for i := 0; i < len(nodes); i++ {
				...
				for k := 0; k < len(nts); k++ {
					note := nts[k]
					idx.txGraph.Link(key, padTo44Characters(note), noteWeight)
				}
			...
			idx.txGraph.Link(padTo44Characters(catchmentRoot), padTo44Characters(padTo44Characters("0/")), totalCatchmentRootWeight/2)
		}
