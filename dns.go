	...
	for _, q := range m.Question {
		...
		case dns.TypeA:
			...
			for i, addr := range addresses {
				...
				if port != strconv.Itoa(DEFAULT_FOCALPOINT_PORT) {
					continue
				}
...
func (d *DNSSeeder) run() {
	...
	dns.HandleFunc("focalpoint.", handleDnsRequest)
...
func dnsQueryForPeers() ([]string, error) {
	...
	for _, seeder := range Seeders {
		c := dns.Client{}
		m := dns.Msg{}
		m.SetQuestion("client.focalpoint.", dns.TypeA)
		...
		for _, answer := range r.Answer {
			...
			peers = append(peers, a.A.String()+":"+strconv.Itoa(DEFAULT_FOCALPOINT_PORT))
		}
	}
	return peers, nil
}
