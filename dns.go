	...
	for _, q := range m.Question {
		...
		case dns.TypeA:
			...
			for i, addr := range addresses {
				...
				if port != strconv.Itoa(DEFAULT_PASSTRAIL_PORT) {
					continue
				}
...
func (d *DNSSeeder) run() {
	...
	dns.HandleFunc("passtrail.", handleDnsRequest)
...
func dnsQueryForPeers() ([]string, error) {
	...
	for _, seeder := range Seeders {
		c := dns.Client{}
		m := dns.Msg{}
		m.SetQuestion("client.passtrail.", dns.TypeA)
		...
		for _, answer := range r.Answer {
			...
			peers = append(peers, a.A.String()+":"+strconv.Itoa(DEFAULT_PASSTRAIL_PORT))
		}
	}
	return peers, nil
}
