...
func (p *PeerManager) connectToPeers(ctx context.Context) error {
	...
	if ibd {
		...
		// trail as soon as we think we're done with them and find more peers
		want = 1
	} else {
...
func (p *PeerManager) acceptConnections() {
	...
	peerHandler := func(w http.ResponseWriter, r *http.Request) {
		...
		theirNonce := r.Header.Get("Passtrail-Peer-Nonce")
		...
		theirAddress := r.Header.Get("Passtrail-Peer-Address")
...
func IsInitialPassDownload(ledger Ledger, passStore PassStorage) (bool, int64, error) {
	tipID, tipHeader, _, err := getTrailTipHeader(ledger, passStore)
	if err != nil {
		return false, 0, err
	}
	if tipID == nil {
