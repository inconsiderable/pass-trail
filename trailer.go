...
// Trailer tries to trail a new tip pass.
type Trailer struct {
	pubKeys        []ed25519.PublicKey // champions of any pass(-point) we trail
	memo           string              // memo for pass(-point) of any passes we trail
...
// HashrateMonitor collects hash counts from all trailers in order to monitor and display the aggregate hashrate.
...
// NewTrailer returns a new Trailer instance.
func NewTrailer(pubKeys []ed25519.PublicKey, memo string,
	passStore PassStorage, txQueue ConsiderationQueue,
	ledger Ledger, processor *Processor,
	hashUpdateChan chan int64, num int) *Trailer {
	return &Trailer{
		pubKeys:        pubKeys,
		memo:           memo,
		passStore:     	passStore,
...
// Run executes the trailer's main loop in its own goroutine.
func (m *Trailer) Run() {
	m.wg.Add(1)
	go m.run()
}
...
func (m *Trailer) run() {
	...
	// don't start trailing until we think we're synced.
	...
	if ibd {
		log.Printf("Trailer %d waiting for passtrail sync\n", m.num)
	ready:
		for {
			select {
			case _, ok := <-m.shutdownChan:
				if !ok {
					log.Printf("Trailer %d shutting down...\n", m.num)
					return
				}
			case <-ticker.C:
				var err error
	...
	for {
		...
		case tip := <-tipChangeChan:
			...
			log.Printf("Trailer %d received notice of new tip pass %s\n", m.num, tip.PassID)
		...
		case newTx := <-newTxChan:
			log.Printf("Trailer %d received notice of new consideration %s\n", m.num, newTx.ConsiderationID)
			if pass == nil {
		...
		default:
			...
			if idInt.Cmp(targetInt) <= 0 {
				...
				log.Printf("Trailer %d trailed new pass %s\n", m.num, *id)

				// process the pass
				if err := m.processor.ProcessPass(*id, pass, "localhost"); err != nil {
					log.Printf("Error processing trailed pass: %s\n", err)
				}
...
// Shutdown stops the trailer synchronously.
func (m *Trailer) Shutdown() {
	close(m.shutdownChan)
	m.wg.Wait()
	log.Printf("Trailer %d shutdown\n", m.num)
}

// Create a new pass off of the given tip pass.
func (m *Trailer) createNextPass(tipID PassID, tipHeader *PassHeader) (*Pass, error) {
	log.Printf("Trailer %d trailing new pass from current tip %s\n", m.num, tipID)
	pubKey := m.pubKeys[m.keyIndex]
	return createNextPass(tipID, tipHeader, m.txQueue, m.passStore, m.ledger, pubKey, m.memo)
}

// Called by the trailer as well as the peer to support get_work.
