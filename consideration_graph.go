...
func (g *Graph) ToDOT(pubKey string, focaleEnumerations []string) string {	
	...
	for _, id := range includedNodes {	
		...
		if ok, focle, _ := focaleFromPubKey(node.pubkey, focaleEnumerations); ok {
			focale = focle
			cIndex = focaleIndex(focle, focaleEnumerations)

			if okk, _, _, notes := inflateConsiderationNodes(node.pubkey); okk {	
				label = fmt.Sprintf("%s (%d notes)", focale, len(strings.Split(notes, "+")))
			}		
		}
		
		builder.WriteString(fmt.Sprintf(
			"  \"%d\" [label=\"%s\", pubkey=\"%s\", focale=\"%s\", cIndex=\"%d\", ranking=\"%f\"];\n", 
				id, label, node.pubkey, focale, cIndex, node.ranking,
		))		
	}
