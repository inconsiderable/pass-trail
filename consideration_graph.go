...
func (g *Graph) ToDOT(pubKey string, catchments []string) string {	
	...
	for _, id := range includedNodes {	
		...
		if ok, chmnt := catchmentFromPubKey(node.pubkey, catchments); ok {
			catchment = chmnt
			cIndex = catchmentIndex(chmnt, catchments)

			if okk, _, _, notes := inflateConsiderationNodes(node.pubkey); okk {	
				label = fmt.Sprintf("%s (%d notes)", catchment, len(strings.Split(notes, "+")))
			}		
		}
		
		builder.WriteString(fmt.Sprintf(
			"  \"%d\" [label=\"%s\", pubkey=\"%s\", catchment=\"%s\", cIndex=\"%d\", ranking=\"%f\"];\n", 
				id, label, node.pubkey, catchment, cIndex, node.ranking,
		))		
	}
